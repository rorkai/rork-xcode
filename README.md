...
- **A view class for every kind.** Each `isa` in the model's vocabulary has its own class, and each class declares the isas it covers through a static `isas` field the view factory dispatches on. Targets of every kind, groups, variant and version groups, synchronized folders and their exception sets, build phases, build rules, build files, configurations and configuration lists, file references, target dependencies, container item proxies, reference proxies, and Swift package references and products all come back typed. Kinds outside the vocabulary fall back to a generic `XcodeObject` with the same read and write access, so nothing in a document is out of reach.
